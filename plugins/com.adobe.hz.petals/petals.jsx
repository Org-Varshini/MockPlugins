    ...
    Adobe.render(
        <>
            ...
            <hz-color-picker
                ...
                wellType="fill"
            />
            <sp-slider label="Petals" onInput={onNumPetals} value={numPetals} min={5} max={27} step={2} />
            <sp-slider
                label="Center Size"
                onInput={onCenterDiameter}
                value={centerDiameter}
                min={0}
                max={maxCenterDiameter}
                formatOptions={pixelFormat}
            />
            <sp-slider
                label="Diameter"
                onInput={onOuterDiameter}
                value={outerDiameter}
                value={outerDiameter}
                min={centerDiameter + 1}
                max={720}
                formatOptions={pixelFormat}
            />

            <div class="buttons">
                <sp-button variant="cta" onClick={createPetals}>
                    Create
                </sp-button>
            </div>
        </>
...
function parseColorStr(colorStr) {
    ...
    const [r, g, b, a] = match[0].split(/(?=(?:..)+$)/).map(x => parseInt(x, 16));
